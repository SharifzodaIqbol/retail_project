...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Монотонно растущий "номер" последнего актуального поиска. Нужен, чтобы
  // отбрасывать ОТВЕТЫ устаревших запросов: _debounce.cancel() отменяет
  // только ещё не сработавший таймер, но если сетевой запрос уже улетел
  // (особенно на слабой связи) и юзер тем временем тапнул по товару в
  // подсказках — очищенный _suggestions через мгновение перезаписывался
  // результатом того самого "старого" запроса, и карточка выбранного
  // товара как будто "повисала" в списке второй раз после выбора.
  int _searchRequestId = 0;

  void _onSearchChanged(String query) {
    if (_debounce?.isActive ?? false) _debounce!.cancel();
    // Любое новое действие (новый ввод, а также явная очистка поля после
    // выбора товара — см. onTap подсказки) делает все ранее запущенные,
    // но ещё не ответившие поисковые запросы неактуальными.
    ...
    _debounce = Timer(const Duration(milliseconds: 150), () async {
      ...
      if (!mounted) return;
      // Пока запрос летел, могла случиться либо новая раскладка (юзер
      // печатает дальше — тогда есть более свежий requestId), либо явный
      // выбор товара из подсказок (тоже увеличивает _searchRequestId) —
      // в обоих случаях этот ответ уже не актуален, применять его нельзя.
  ...
  void _onManualSearchSubmit() {
    ...
    // См. комментарий в onTap подсказки — инвалидируем ещё не ответившие
    // поисковые запросы, чтобы их результат не перезаписал уже очищенный
    // список подсказок.
    _searchRequestId++;
    setState(() => _suggestions = []);
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        return ListTile(
                          ...
                          onTap: () {
                            setState(() {
                              _suggestions = [];
                              // Явно инвалидируем поисковые запросы,
                              // которые могли ещё лететь в фоне (см.
                              // _searchRequestId) — без этого ответ
                              // "старого" поиска, прилетевший ПОСЛЕ выбора
                              // товара, мог вернуть только что очищенный
                              // список подсказок обратно.
                              _searchRequestId++;
                              _searchController.clear();
                            });
                            _addProductSmart(p);
