...
class _LabelLayout {
  ...
  // 4 колонки × 7 строк, ячейка ~48.5×33.9 мм — компактнее, больше
  // этикеток на листе.
  // ИСПРАВЛЕНО: раньше здесь было 6 строк, что занимало только 247.4 мм
  // из 297 мм высоты A4 — почти 50 мм (целый ряд этикеток) внизу листа
  // просто пустовали. При тех же размерах ячейки 7 строк умещаются с
  // запасом (281.3 мм из 297), давая 28 этикеток на лист вместо 24.
  ...
  // 2 колонки × 5 строк, крупная этикетка.
  // ИСПРАВЛЕНО: при исходной высоте ячейки 57 мм сетка требовала
  // 312 мм высоты (5×57 + отступы 13.5×2), а лист A4 — только 297 мм.
  // Нижний ряд (2 этикетки на странице) физически выходил за пределы
  // листа и не печатался — отсюда несовпадение числа выбранных и
  // реально напечатанных этикеток. Уменьшили высоту ячейки до 56 мм и
  // отступ до 8 мм — сетка 5×56 + 8×2 = 296 мм укладывается в лист.
  static final grid2x5 = _LabelLayout(
    title: '2×5 (99.1×56 мм)',
    columns: 2,
    rows: 5,
    cellWidth: 99.1 * _mm,
    cellHeight: 56 * _mm,
