...
func main() {
	...
	api.Use(middleware.AuthMiddleware(jwtSecret)) // Проверка JWT-токена
	{
		...
		api.GET("/sales", func(c *gin.Context) {
			// Здесь должен быть вызов метода репозитория для получения всех продаж
			sales, err := saleRepo.GetAll(context.Background())
			if err != nil {
				log.Println(err)
		...
		api.GET("/products/search", func(c *gin.Context) {
			name := c.Query("name")
			products, err := productRepo.SearchByName(context.Background(), name)
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
				return
			}
