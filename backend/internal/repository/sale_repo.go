...
func (r *SaleRepository) GetTodayTotal(ctx context.Context) (float64, error) {
	...
	query := `SELECT COALESCE(SUM(total_amount), 0) FROM sales WHERE created_at > CURRENT_DATE AND is_canceled = false`
...
func (r *SaleRepository) GetAll(ctx context.Context) ([]domain.Sale, error) {
    // 1. Пишем запрос (сортируем по ID убыванию, чтобы новые чеки были сверху)
    query := `
        SELECT id, seller_id, total_amount, is_canceled, cancel_reason 
        FROM sales 
        ORDER BY id DESC`
